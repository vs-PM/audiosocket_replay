...
async def handle_client(reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
    ...
    try:
        # === Первичный пакет (инициализация с UUID) ===
        ...
        packet_counter = 0
        while True:
            # === ВНИМАНИЕ: заголовок AudioSocket теперь 1 (тип) + 2 (длина) ===
            ...
            if pkt_type == AUDIO_PACKET_TYPE:
                if payload_length >= 16:
                    ...
                    audio = payload[16:]
                else:  # если payload неверный по длине
                    uuid, audio = None, payload
                    logging.warning("Payload AUDIO слишком короток!")

                total_audio_bytes += len(audio)
                write_all_audio(audio)
                await broadcast_audio(uuid or session_uuid, audio)

            elif pkt_type == HEARTBEAT_PACKET_TYPE:
                logging.debug(f"Хартбит [{session_uuid}]")
...
async def run_audiosocket_server(port=None):
    ...
    setup_audio_file()
    server = await asyncio.start_server(handle_client, "0.0.0.0", port, reuse_address=True, start_serving=True)
